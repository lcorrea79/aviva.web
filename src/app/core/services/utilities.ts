...
export class Utilities {
  ...
  public static searchArray(searchTerm: any, caseSensitive: boolean, ...values: any[]) {
    if (!searchTerm) {
      return true;
    }
   ...
   * @param timeString Time string to be converted. Format: HH:MM AM/PM
   * @returns Date object
   **************************************************************************************************/
  public static parseTime(date: Date, timeString: string) {
    const timeParts = timeString.split(' ');
    const amOrPm = timeParts[1];
    const [hoursString, minutesString] = timeParts[0]?.split(':');
    ...
    if (amOrPm.toUpperCase() === 'AM' && hours === 12) {
      hours = 0;
    }
  ...
  public static createDateFromTimeString(time: string): Date | null {
    if (!time) return null;

    const [hours, minutes] = time.split(':').map(Number);
    const date = new Date();
    date.setHours(hours, minutes, 0, 0); // Set time and reset seconds and milliseconds

    return date;
  }

  // Helper method to convert a date object to a time string
  public static createTimeStringFromDate(date: Date): string {
    if (!date) return '';
